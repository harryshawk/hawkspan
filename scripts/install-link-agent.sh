...
template_path="$active_root/launchd/org.hawkspan.link-agent.plist.template"
target_path="$HOME/Library/LaunchAgents/org.hawkspan.link-agent.plist"
uid_value=$(id -u)
...
trap - EXIT
launchctl bootout "gui/$uid_value/org.hawkspan.link-agent" 2>/dev/null || true
sleep 1
launchctl bootstrap "gui/$uid_value" "$target_path" || {
  sleep 1
  launchctl bootstrap "gui/$uid_value" "$target_path"
}
launchctl enable "gui/$uid_value/org.hawkspan.link-agent"
launchctl kickstart -k "gui/$uid_value/org.hawkspan.link-agent"
echo "$target_path"
